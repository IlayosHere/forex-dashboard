...
from config import get_spread_pips
...
def calculate_trade_params(signal: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # Approximate broker hour from UTC (UTC+2, conservative winter offset).
    # H0/H1 spread tiers only kick in near broker midnight (00:00-01:59).
    broker_hour = (candle_time.hour + 2) % 24
    ...
    # TP at 1:1: same pip distance as effective risk from entry close
    if direction == "BUY":
        tp = close + effective_risk_pips * pip
    else:
        tp = close - effective_risk_pips * pip
