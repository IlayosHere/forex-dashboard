...
# SQLite stores booleans as integers; these need casting to bool for Postgres
BOOLEAN_COLUMNS: dict[str, set[str]] = {
    "trades": {"ict_smt_present", "ict_tdo_aligned"},
}


def cast_row(row: tuple, columns: list[str], table: str) -> tuple:
...
def migrate_table(
    ...
    # signals has a dedup constraint on (strategy, symbol, candle_time) in addition to id
    ...
    if table == "signals":
        conflict_clause = "ON CONFLICT ON CONSTRAINT uq_signal_dedup DO UPDATE SET " + update_set
    else:
    ...
    rows = [cast_row(r, columns, table) for r in rows]
    cursor = dst.cursor()
